...
> Ne jamais utiliser `libxml_disable_entity_loader()` (dépréciée, émet E_DEPRECATED sur toutes les versions PHP 8.x supportées). Ne jamais passer `LIBXML_NOENT` ou `LIBXML_DTDLOAD` à des parseurs XML traitant des données utilisateur.
...
nelmio_security:
    ...
    # XSS: handled by Content-Security-Policy above (X-XSS-Protection is deprecated — removed from Chrome 78+/Firefox/Edge)
