...
| **Versioning** | Semantic versioning modèles (1.2.3) |
...
- **Feature reuse** — feature store pour éviter duplication
- **Model versioning** — registry centralisé, jamais de model.pkl local
- **Shadow mode** — déployer nouveau modèle en shadow avant switch
- **Monitoring continu** — drift data + drift model + latency
- **A/B testing** — comparer modèles en prod avec métriques business

## Patterns Déploiement
...
| **Shadow** | Nouveau modèle log prédictions sans servir | Validation comportement |
| **A/B Testing** | Split traffic entre 2 modèles | Optimisation métrique business |
