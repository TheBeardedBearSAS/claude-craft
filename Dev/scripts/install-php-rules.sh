...
TECH_NAME="PHP"
TECH_NAMESPACE="php"
DEFAULT_STACK="PHP 8.4+, Symfony/Laravel, Clean Architecture, PHPUnit, PHPStan, Doctrine"
lang="en"
...
Description:
    ...
    - PHP 8.4+, PSR-12, PHPStan level 9
    - PHPUnit/Pest, Tests unitaires et integration
    - OWASP Security, CQRS pattern
EOF
}
...
copy_common_rules() {
    ...
    if [ "$dry_run" = "false" ] && [ $count -gt 0 ]; then
        log_success "$count PHP-specific rules copied"
    fi
}
...
process_templates() {
    ...
    local escaped_tech_stack="${tech_stack//\//\\/}"
    ...
    if [ "$dry_run" = "true" ]; then
        ...
        if [ -f "$claude_md" ] && [ "$preserve_config" = "true" ]; then
            ...
            sed -e "s/{{PROJECT_NAME}}/${project_name}/g" \
                -e "s/{{TECH_STACK}}/${escaped_tech_stack}/g" \
...
show_summary() {
    ...
    echo "  /${TECH_NAMESPACE}:check-compliance     Audit conformite (score /100)"
    echo "  /${TECH_NAMESPACE}:check-architecture   Clean Architecture validation"
    echo "  /${TECH_NAMESPACE}:check-code-quality   Qualite code (PHPStan, Psalm)"
    echo "  /${TECH_NAMESPACE}:check-testing        Tests et couverture"
