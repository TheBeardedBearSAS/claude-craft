...
Description:
    ...
    - Standards PSR-12, PHP 8.3+ attributes
    ...
    Reduction tokens: ~95% (de ~120K a ~5K)
EOF
}
...
**Always**: PSR-12, PHP 8.3+ attributes, strict types, final classes."
    ...
    local testing_stack="**Symfony Stack**: PHPUnit + Codeception + Behat + Infection (mutation testing)"

    local tech_references="- \`${TECH_NAMESPACE}/architecture.md\` - Clean Architecture + DDD
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --version) echo "install-symfony-rules.sh version ${VERSION}"; exit 0 ;;
