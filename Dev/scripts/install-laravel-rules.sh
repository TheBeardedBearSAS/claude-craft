...
DEFAULT_STACK="Laravel 12, PHP 8.3+, Clean Architecture, Pest PHP, PostgreSQL, Redis"
lang="en"
...
Description:
    ...
    - Laravel 12, PHP 8.3+, Modern Patterns
    - Eloquent, Actions, DTOs, Form Requests
    - Pest PHP, PHPStan, Laravel Pint
    - OWASP Security, Sanctum Authentication
EOF
}
...
copy_common_rules() {
    ...
    if [ "$dry_run" = "false" ] && [ $count -gt 0 ]; then
        log_success "$count Laravel-specific rules copied"
    fi
}
    ...
    if [ "$dry_run" = "true" ]; then
        ...
        if [ -f "$claude_md" ] && [ "$preserve_config" = "true" ]; then
            ...
            sed -e "s/{{PROJECT_NAME}}/${project_name}/g" \
                ...
                -e "s/{{LARAVEL_VERSION}}/12/g" \
                -e "s/{{PHP_VERSION}}/8.3/g" \
                -e "s/{{ARCHITECTURE_PATTERN}}/Clean Architecture/g" \
                -e "s/{{DATABASE_TYPE}}/PostgreSQL/g" \
        ...
        if [ -f "$project_context" ] && [ "$preserve_config" = "true" ]; then
            ...
            sed -e "s/{{PROJECT_NAME}}/${project_name}/g" \
                ...
                -e "s/{{ARCHITECTURE_PATTERN}}/Clean Architecture/g" \
                -e "s/{{DATABASE_TYPE}}/PostgreSQL/g" \
                -e "s/{{DATABASE_VERSION}}/16/g" \
                -e "s/{{REDIS_VERSION}}/7/g" \
                -e "s/{{QUEUE_DRIVER}}/redis/g" \
...
show_summary() {
    ...
    echo ""
    echo "=========================================="
    echo "Installation Laravel terminee!"
    echo "=========================================="
    echo ""
    ...
    echo "  /${TECH_NAMESPACE}:check-code-quality   Qualite code (PHPStan, Pint)"
    echo "  /${TECH_NAMESPACE}:check-testing        Tests (Pest PHP)"
    echo "  /${TECH_NAMESPACE}:check-security       Securite (OWASP)"
    echo "  /${TECH_NAMESPACE}:generate-controller  Generer controller API"
    echo ""
...
main() {
    ...
    echo ""
    echo "Installation des regles Claude Code - Laravel"
    echo "=========================================="
