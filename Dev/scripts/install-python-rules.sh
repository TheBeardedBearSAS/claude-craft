...
copy_tech_skills() {
    ...
    if [ "$dry_run" = "false" ] && [ $count -gt 0 ]; then
        log_success "$count Python-specific skills copied"
    fi
}
...
**Always**: Type hints, docstrings, ruff formatting."
    ...
    local testing_stack="**Python Stack**: pytest + pytest-cov + pytest-asyncio + Faker + testcontainers"

    local tech_references="- \`${TECH_NAMESPACE}/architecture.md\` - Clean Architecture for Python
...
- \`${TECH_NAMESPACE}/security.md\` - OWASP security guidelines"
    ...
    local file_contexts="  # Python source files
  \"*.py\":
    suggest_skills:
      - solid-principles
      - kiss-dry-yagni
