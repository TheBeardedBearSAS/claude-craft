...
DEFAULT_STACK="Angular 18+, TypeScript 5+, Standalone Components, Signals, Vitest, Tailwind"
lang="en"
...
Installation/Mise a jour des regles Claude Code pour projets Angular TypeScript.
...
copy_common_rules() {
    ...
    if [ "$dry_run" = "false" ] && [ $count -gt 0 ]; then
        log_success "$count Angular-specific rules copied"
    fi
}
...
show_summary() {
    ...
    echo "  /${TECH_NAMESPACE}:generate-component   Generer composant"
    echo ""
