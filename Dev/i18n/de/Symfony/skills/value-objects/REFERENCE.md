...
Ein Value Object repräsentiert ein **unveränderliches** Business-Konzept **ohne Identität**. Zwei VOs mit denselben Werten gelten als gleich.

## Verpflichtende Eigenschaften

1. **Unveränderlich** - Keine Änderung nach Erstellung
2. **Final + Readonly** - Klassen nicht erweiterbar
3. **Validierung im Konstruktor** - Fail-fast
4. **Wertegleichheit** - `equals()` Methode
5. **Type-safe** - Keine primitiven Werte in Domain
...
{
    public function __construct(
        private int $amountInCents,  // In Cent speichern!
...
## Value Objects Checkliste

- [ ] Klasse `final readonly`
- [ ] Validierung im Konstruktor
- [ ] Statische Factory-Methode `fromXxx()`
- [ ] `getValue()` Methode oder spezifische Getter
- [ ] `equals()` Methode für Vergleich
- [ ] `__toString()` Methode wenn relevant
- [ ] Keine Setter (Unveränderlichkeit)
- [ ] Custom Doctrine Type erstellt
- [ ] Unit-Tests decken Validierung ab
- [ ] Dokumentation der erwarteten Formate
