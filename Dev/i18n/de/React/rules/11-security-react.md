...
```typescript
// ❌ SCHLECHT - Using user input directly
...
// ✅ GUT - Validate and sanitize input
import { z } from 'zod';
...
```typescript
// ❌ SCHLECHT - localStorage is vulnerable to XSS
localStorage.setItem('token', jwtToken);

// ❌ SCHLECHT - sessionStorage too
sessionStorage.setItem('token', jwtToken);

// ✅ GUT - HttpOnly Cookie (managed by server)
...
add_header Content-Security-Policy "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data: https:; font-src 'self'; connect-src 'self' https://api.example.com; frame-ancestors 'none'; upgrade-insecure-requests;" always;
...
```typescript
// ❌ SCHLECHT - NEVER commit secrets
const API_KEY = 'sk-1234567890abcdef'; // DANGER!

// ✅ GUT - Use environment variables
