...
- Rückgabe `Either<Failure, Success>` (mit **fpdart** — `dartz` ist nicht mehr gepflegt) oder die native `Result<T>` sealed class (empfohlen)
- Async mit `Future`
...
**Alternative mit Result<T>** (native sealed class — empfohlen für Dart 3.x):
