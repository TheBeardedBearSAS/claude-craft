...
Produce a well-formed payload for hiring a Paperclip agent. Paperclip (v2026.609.0) does **not** hire agents from a `.yaml` file via CLI — hiring happens through the dashboard or `POST /companies/:companyId/agents`. This command drafts the JSON payload and walks the operator through filling it in.
...
> **Check the real shape.** Before POSTing, open `server/src/routes/agents.ts` (or the OpenAPI spec served by the instance) to confirm the exact schema — the above reflects what was observed at v2026.609.0 but field names can evolve.
