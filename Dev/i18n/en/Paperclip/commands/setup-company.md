...
> The real `paperclipai` CLI (v2026.609.0) does **not** expose a `companies create` command. Company creation happens either through the dashboard or by importing a package with `paperclipai company import`. Don't invent flags that don't exist — open `paperclipai company --help` and follow what's there.
...
Paperclip ships with built-in adapters (observed v2026.609.0):
...
Paperclip does **not** hire agents from a YAML file via CLI (at v2026.609.0). Hire an agent:
