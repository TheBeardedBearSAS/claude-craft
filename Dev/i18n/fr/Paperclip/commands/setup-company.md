...
> Le vrai CLI `paperclipai` (v2026.609.0) n'expose **pas** de commande `companies create`. La création d'entreprise se fait soit via le tableau de bord, soit en important un package avec `paperclipai company import`. Ne pas inventer de flags qui n'existent pas — ouvrir `paperclipai company --help` et suivre ce qui est là.
...
Paperclip est livré avec des adaptateurs intégrés (observés v2026.609.0) :
...
Paperclip n'embauche **pas** d'agents depuis un fichier YAML via CLI (à la v2026.609.0). Embaucher un agent :
