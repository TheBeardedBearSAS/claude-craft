...
```
              /\
             /  \
            / E2E\     <- Peu de tests, lents, couvrent les parcours critiques
           /------\
          /        \
         /Integration\ <- Tests de plusieurs composants ensemble
        /------------\
       /              \
...
- **20%** : Tests d'intégration (features complètes)
...
> **Version courante : Playwright 1.60.0** — Nouvelles fonctionnalités : fixtures composables, `toMatchAriaSnapshot()` pour les assertions d'accessibilité, serveur MCP pour intégration Claude Code.
...
│       ├── hooks/
│       │   └── useUsers.test.ts
│       └── UserManagement.integration.test.tsx  # Tests d'intégration
│
├── test/
...
```typescript
// 1. Accessible to everyone (meilleur)
...
Une stratégie de tests complète assure :

1. ✅ **Confiance** : Le code fonctionne comme prévu
2. ✅ **Documentation** : Les tests documentent le comportement
3. ✅ **Refactoring** : Modifier sans casser
4. ✅ **Qualité** : Détecter les bugs tôt
5. ✅ **Régression** : Éviter les régressions

**Règle d'or** : Écrire des tests qui testent le comportement utilisateur, pas l'implémentation technique.
