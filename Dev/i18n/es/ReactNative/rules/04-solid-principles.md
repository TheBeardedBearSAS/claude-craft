# Principios SOLID en React Native

## Introducción

Los principios SOLID son esenciales para crear aplicaciones React Native mantenibles, probables y escalables. Este documento adapta estos principios al contexto de React Native con TypeScript.
...
Cada entidad debe tener una sola responsabilidad bien definida.

### Aplicación en React Native

#### ❌ MALO: Componente con múltiples responsabilidades

```typescript
// UserProfile.tsx - Hace DEMASIADAS cosas
export const UserProfile = () => {
  ...
  // Responsabilidad 1: Data fetching
  useEffect(() => {
  ...
  // Responsabilidad 2: Form handling
  const handleChange = (field: string, value: string) => {
    setFormData({ ...formData, [field]: value });
  };

  // Responsabilidad 3: Form submission
  ...
  // Responsabilidad 4: UI rendering
  return (
    <View>
...
// 3. UI Layer - Componentes de presentación
...
- **Probabilidad**: Cada parte se puede probar independientemente
- **Reutilizabilidad**: Componentes y hooks reutilizables
- **Mantenibilidad**: Cambios aislados
- **Legibilidad**: Código claro y enfocado
...
Debemos poder extender el comportamiento sin modificar el código existente.

### Aplicación en React Native

#### ❌ MALO: Modificación del componente para cada variante

```typescript
// Button.tsx - Requiere modificación para cada nueva variante
export const Button = ({ variant, onPress, children }: ButtonProps) => {
  ...
  // Agregar una nueva variante = modificar este código ❌
...
#### ✅ BUENO: Extensión mediante composición y configuración

```typescript
// 1. Base Button - cerrado a modificación
...
// 2. Variants via composición - abierto a extensión
...
// 3. Nueva variante = nuevo componente, sin modificación ✅
...
#### Ejemplo: Abstracción de Storage

```typescript
// 1. Interfaz - cerrada a modificación
...
// 2. Implementaciones - abiertas a extensión
...
// 3. Nueva implementación sin modificar lo existente ✅
...
export class SecureStorageAdapter implements IStorage {
  ...
  async clear(): Promise<void> {
    // SecureStore no tiene clear, debemos rastrear las keys
    // Implementación...
  }
}
...
// Implementación que viola el contrato
export const IconButton: FC<IButton> = ({ onPress }) => {
  // ❌ Ignora children, viola el contrato IButton
  return (
...
interface IconButtonProps extends BaseButtonProps {
  icon: string;
  label?: string; // Opcional, no children
}
...
// Implementación ScrollView (para listas pequeñas)
...
// ✅ Intercambiables porque respetan el contrato
...
Es mejor tener varias interfaces específicas que una gran interfaz genérica.

### Aplicación en React Native

#### ❌ MALO: Interfaz "gorda" con props innecesarias
...
// Componente forzado a aceptar todas las props
export const ArticleCard: FC<ArticleCardProps> = ({
  article,
  onPress,
  onEdit,
  onDelete, // ❌ No usado en este contexto
  onShare, // ❌ No usado en este contexto
  onFavorite,
  onComment, // ❌ No usado en este contexto
  showAuthor,
  showDate,
  showTags,
  showStats, // ❌ No usado en este contexto
  compact,
}) => {
  // Implementación que no usa todas las props
};
```
...
// 2. Interfaces específicas componibles
...
// 3. Componentes específicos con solo las props necesarias
...
// Uso - cada contexto usa el componente apropiado
// Lista simple
...
#### Ejemplo: Componentes de formulario

```typescript
// Interfaces segregadas para formularios
...
// Componentes usando solo las props necesarias
...
#### ❌ MALO: Dependencia directa de implementaciones

```typescript
// Screen depende directamente de la implementación concreta
export default function ArticlesScreen() {
  const [articles, setArticles] = useState<Article[]>([]);

  useEffect(() => {
    // ❌ Dependencia directa de la API
  ...
  // ❌ Dependencia directa del storage
...
// 3. Capa de servicio usando abstracciones
...
// 5. Hooks abstraen los servicios
...
// 6. Screen depende de abstracciones (hooks)
export default function ArticlesScreen() {
...
1. **Probabilidad**: Inyección de mocks fácil
2. **Flexibilidad**: Cambio de implementación sin impacto
3. **Desacoplamiento**: Componentes independientes de los detalles de implementación
...
| Principio | Definición | Beneficio Principal |
|----------|-----------|-------------------|
| **SRP** | Una sola responsabilidad | Mantenibilidad |
| **OCP** | Abierto/Cerrado | Extensibilidad |
| **LSP** | Sustitución | Fiabilidad |
| **ISP** | Interfaces segregadas | Simplicidad |
| **DIP** | Inversión de dependencias | Probabilidad |
...
- [ ] Extensión posible sin modificación
- [ ] Los subtipos respetan los contratos
- [ ] No hay props/métodos no utilizados
- [ ] Dependencias mediante interfaces/abstracciones
- [ ] Inyección de dependencias utilizada
- [ ] Código proble con mocks
- [ ] Separación clara de capas

---

**SOLID no es una restricción, es una guía hacia un código de calidad.**
