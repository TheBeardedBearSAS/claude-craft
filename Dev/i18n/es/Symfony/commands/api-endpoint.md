---
description: Creación Endpoint API Platform
argument-hint: [arguments]
---

# Creación Endpoint API Platform

Eres un desarrollador senior de Symfony y API Platform. Debes crear un endpoint API REST completo con validación, documentación OpenAPI, paginación y pruebas.

## Argumentos
$ARGUMENTS

Argumentos:
- Recurso (nombre de la entidad)
- Operaciones (list, get, post, put, patch, delete)

Ejemplo: `/symfony:api-endpoint Product "list,get,post,patch,delete"`

## MISIÓN

### Paso 1: Configurar el Recurso API Platform

#### Entity con Atributos de API Platform
...
### Paso 2: State Processors (para lógica de negocio)

#### Processor para la creación
    ...
    public function getDescription(string $resourceClass): array
    {
        return [
            'activeOnly' => [
                ...
                'type' => 'bool',
                'required' => false,
                'description' => 'Filtrar únicamente los elementos publicados',
            ],
        ];
    }
}
...
class {Entity}ApiTest extends ApiTestCase
{
    ...
    public function testGetCollection(): void
    {
        ...
        $this->assertCount(20, $response->toArray()['hydra:member']); // Paginación
    }
...
```
══════════════════════════════════════════════════════════════
✅ ENDPOINT API PLATFORM CREADO - {Entity}
══════════════════════════════════════════════════════════════

📁 Archivos creados/modificados:
- src/Entity/{Entity}.php (con atributos de API Platform)
...
|---------|-----|-------------|------|
| GET | /api/{entities} | Lista paginada | No |
| GET | /api/{entities}/{id} | Detalle | No |
| POST | /api/{entities} | Creación | ROLE_USER |
| PATCH | /api/{entities}/{id} | Modificación | Owner |
| DELETE | /api/{entities}/{id} | Eliminación | ROLE_ADMIN |

🔍 Filtros disponibles:
- ?name=xxx (búsqueda parcial)
