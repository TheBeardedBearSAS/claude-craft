...
Producir un payload bien formado para contratar un agente Paperclip. Paperclip (v2026.609.0) **no** contrata agentes desde un archivo `.yaml` vía CLI — la contratación ocurre a través del dashboard o `POST /companies/:companyId/agents`. Este comando redacta el payload JSON y guía al operador a llenarlo.
...
> **Verificar la forma real.** Antes de POSTear, abrir `server/src/routes/agents.ts` (o la especificación OpenAPI servida por la instancia) para confirmar el esquema exacto — lo anterior refleja lo observado en v2026.609.0 pero los nombres de campos pueden evolucionar.
