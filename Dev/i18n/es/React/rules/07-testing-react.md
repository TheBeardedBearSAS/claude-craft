...
```
              /\
             /  \
            / E2E\     <- Pocos tests, lentos, cubren rutas críticas
           /------\
          /        \
         /Integración\ <- Tests de múltiples componentes juntos
        /------------\
       /              \
      /    Unitarios  \ <- Muchos tests, rápidos, aislados
     /------------------\
```

### Distribución Recomendada

- **70%**: Tests unitarios (componentes, hooks, utils)
- **20%**: Tests de integración (características completas)
- **10%**: Tests E2E (recorridos críticos del usuario)
...
### Configuración (src/test/mocks/handlers.ts)
...
### Configuración del Servidor (src/test/mocks/server.ts)
...
// Reiniciar handlers después de cada test
...
describe('UserList', () => {
  ...
  it('should display error message on fetch error', async () => {
    // Sobrescribir handler para este test
    server.use(
      http.get('/api/users', () => {
        return new HttpResponse(null, { status: 500 });
      })
...
describe('UserManagement Integration', () => {
  it('should complete full user creation flow', async () => {
    ...
    // Hacer clic en botón agregar usuario
  ...
  it('should complete full user edit flow', async () => {
    ...
    // Hacer clic en editar primer usuario
  ...
  it('should complete full user delete flow', async () => {
    ...
    // Hacer clic en eliminar primer usuario
...
> **Versión actual: Playwright 1.60.0** — Nuevas funciones: fixtures componibles, `toMatchAriaSnapshot()` para aserciones de accesibilidad, servidor MCP para integración con Claude Code.
...
test.describe('Authentication', () => {
  ...
  test('should logout successfully', async ({ page }) => {
    ...
    // Verificar que el botón de login es visible
...
│       ├── hooks/
│       │   └── useUsers.test.ts
│       └── UserManagement.integration.test.tsx  # Tests integración
│
├── test/
...
│   ├── utils/
│   │   └── customRender.tsx        # Helpers de test
│   └── factories/
│       └── user.factory.ts         # Factories de datos
│
...
### 2. Testear Comportamiento del Usuario, No Implementación
...
### 3. Prioridad de Queries

```typescript
// 1. Accesibles para todos (mejor)
...
Una estrategia de testing completa garantiza:

1. ✅ **Confianza**: El código funciona como se espera
2. ✅ **Documentación**: Los tests documentan el comportamiento
3. ✅ **Refactorización**: Modificar sin romper
4. ✅ **Calidad**: Detectar bugs temprano
5. ✅ **Regresión**: Prevenir regresiones

**Regla de oro**: Escribir tests que testean el comportamiento del usuario, no la implementación técnica.
