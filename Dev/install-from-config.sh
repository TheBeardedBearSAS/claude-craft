...
list_projects() {
    ...
    for ((i=0; i<project_count; i++)); do
        local name description root common module_count
        name=$(yq e ".projects[$i].name" "$config_file")
...
get_install_script() {
    ...
    case "$tech" in
        symfony)
            script="${SCRIPT_DIR}/Symfony/install-symfony-rules.sh"
            ;;
        flutter)
            script="${SCRIPT_DIR}/Flutter/install-flutter-rules.sh"
            ;;
        python)
            script="${SCRIPT_DIR}/Python/install-python-rules.sh"
            ;;
        react)
            script="${SCRIPT_DIR}/React/install-react-rules.sh"
            ;;
        reactnative)
            script="${SCRIPT_DIR}/ReactNative/install-reactnative-rules.sh"
            ;;
        common)
            script="${SCRIPT_DIR}/Common/install-common-rules.sh"
            ;;
    esac
...
install_project() {
    ...
    if [[ "$common" == "true" ]]; then
        ...
        install_module "$expanded_root" "common" ""
        ((step++))
    fi
    ...
    for ((j=0; j<module_count; j++)); do
        ...
        install_module "$target_path" "$tech" "$desc"
        ((step++))
    done
}
