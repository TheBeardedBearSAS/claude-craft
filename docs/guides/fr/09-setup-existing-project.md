...
```bash
cd ~/claude-craft
make install-symfony TARGET=~/votre-projet-existant RULES_LANG=fr
```
...
make install-symfony TARGET=~/votre-projet-existant RULES_LANG=fr OPTIONS="--preserve-config"
```
