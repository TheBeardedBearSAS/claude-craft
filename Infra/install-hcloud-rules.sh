#!/bin/bash
# Multilingual installation script for Claude Code Infrastructure rules (Hcloud)
# Version: 1.0.0
# Usage: ./install-hcloud-rules.sh [OPTIONS] [PROJECT_DIR]
#
# Installs Hcloud agents and commands for infrastructure automation
...
Install/Update Hcloud agents and commands for Claude Code.
...
Examples:
    ./install-hcloud-rules.sh --install ./my-project
    ./install-hcloud-rules.sh --update
    ./install-hcloud-rules.sh --lang=fr --install ./project
    ./install-hcloud-rules.sh --dry-run

Description:
    This script installs Hcloud agents and commands for Claude Code.

    5 agents:
    - @hcloud-architect       - Hetzner Cloud infrastructure architecture designer
    - @hcloud-deployment      - Hetzner Cloud CI/CD and deployment pipeline specialist
    - @hcloud-debug           - Hetzner Cloud troubleshooting specialist
    - @hcloud-security        - Hetzner Cloud security and firewall specialist
    - @hcloud-cost            - Hetzner Cloud cost optimization and right-sizing specialist
    ...
    - /hcloud:debug             - Diagnose Hetzner Cloud infrastructure issues
...
show_version() {
    echo "install-hcloud-rules.sh version ${VERSION}"
    echo "Hcloud agents and commands for Claude Code"
}
...
create_backup() {
    local target_dir="$1"
    local dry_run="$2"
    local backup_dir="${target_dir}/.claude-backup-hcloud-$(date +%Y%m%d-%H%M%S)"

    if [ -d "${target_dir}/.claude/commands/${TECH_NAMESPACE}" ] || [ -d "${target_dir}/.claude/agents" ]; then
        if [ "$dry_run" = "true" ]; then
            log_dry_run "Backup Hcloud files to ${backup_dir}"
        else
            mkdir -p "${backup_dir}/commands"
            mkdir -p "${backup_dir}/agents"
            cp -r "${target_dir}/.claude/commands/${TECH_NAMESPACE}" "${backup_dir}/commands/" 2>/dev/null || true
            # Only backup hcloud-related agents
            for agent in hcloud-architect hcloud-deployment hcloud-debug hcloud-security hcloud-cost; do
...
copy_agents() {
    ...
    if [ -d "$agt_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} Hcloud agents copied"
        fi
    fi
}
...
copy_commands() {
    ...
    if [ -d "$cmd_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} Hcloud commands copied"
        fi
    fi
}
...
show_summary() {
    ...
    echo "  |   |-- agents/             (${agents_count} Hcloud agents)"
    echo ""

    # List available commands
    if [ $commands_count -gt 0 ]; then
        echo "Available Hcloud commands:"
    ...
    echo "Usage examples:"
    echo "  @hcloud-architect       Design Hetzner Cloud infrastructure"
    echo "  @hcloud-security        Audit Hetzner Cloud security posture"
    echo "  /hcloud:architecture    Design complete Hetzner Cloud architecture"
    echo "  /hcloud:debug           Diagnose Hetzner Cloud issues"
    echo "  /hcloud:deploy-setup    Setup Hetzner Cloud CI/CD pipeline"
    echo ""
}
...
main() {
    ...
    echo ""
    echo "=========================================="
    echo "Installing Hcloud rules for Claude Code"
    echo "=========================================="
    echo ""
    ...
    if [ -z "$mode" ]; then
        ...
        case $installation_status in
            existing)
                ...
                ;;
            partial|none)
                log_info "No Hcloud installation detected -> install mode"
                mode="install"
                ;;
        esac
    fi
    ...
    case $mode in
        install|update)
            log_info "Installing Hcloud agents and commands..."
