#!/bin/bash
# Multilingual installation script for Claude Code Infrastructure rules (Ansible)
# Version: 1.0.0
# Usage: ./install-ansible-rules.sh [OPTIONS] [PROJECT_DIR]
#
# Installs Ansible agents and commands for infrastructure automation
...
Install/Update Ansible agents and commands for Claude Code.
...
Examples:
    ./install-ansible-rules.sh --install ./my-project
    ./install-ansible-rules.sh --update
    ./install-ansible-rules.sh --lang=fr --install ./project
    ./install-ansible-rules.sh --dry-run

Description:
    This script installs Ansible agents and commands for Claude Code.

    5 agents:
    - @ansible-architect      - Ansible project architecture and role designer
    - @ansible-deployment     - Ansible CI/CD and pipeline automation specialist
    - @ansible-debug          - Ansible playbook troubleshooting specialist
    - @ansible-security       - Ansible Vault and secrets management specialist
    - @ansible-quality        - Ansible linting, testing, and quality specialist

    5 commands:
    - /ansible:architecture     - Design complete Ansible automation architecture
    - /ansible:deploy-setup     - Setup CI/CD pipeline for Ansible
    - /ansible:debug            - Diagnose Ansible playbook issues
    - /ansible:security-audit   - Audit Ansible security posture
    - /ansible:optimize         - Optimize Ansible performance and quality
EOF
}

show_version() {
    echo "install-ansible-rules.sh version ${VERSION}"
    echo "Ansible agents and commands for Claude Code"
}
...
create_backup() {
    local target_dir="$1"
    local dry_run="$2"
    local backup_dir="${target_dir}/.claude-backup-ansible-$(date +%Y%m%d-%H%M%S)"

    if [ -d "${target_dir}/.claude/commands/${TECH_NAMESPACE}" ] || [ -d "${target_dir}/.claude/agents" ]; then
        if [ "$dry_run" = "true" ]; then
            log_dry_run "Backup Ansible files to ${backup_dir}"
        else
            mkdir -p "${backup_dir}/commands"
            mkdir -p "${backup_dir}/agents"
            cp -r "${target_dir}/.claude/commands/${TECH_NAMESPACE}" "${backup_dir}/commands/" 2>/dev/null || true
            # Only backup ansible-related agents
            for agent in ansible-architect ansible-deployment ansible-debug ansible-security ansible-quality; do
...
copy_agents() {
    ...
    if [ -d "$agt_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} Ansible agents copied"
        fi
    fi
}
...
copy_commands() {
    ...
    if [ -d "$cmd_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} Ansible commands copied"
        fi
    fi
}
...
show_summary() {
    ...
    echo "  |   |-- agents/             (${agents_count} Ansible agents)"
    echo ""
    ...
    echo "Usage examples:"
    echo "  @ansible-architect      Design Ansible project architecture"
    echo "  @ansible-security       Audit Ansible security posture"
    echo "  /ansible:architecture   Design complete Ansible architecture"
    echo "  /ansible:debug          Diagnose Ansible playbook issues"
    echo "  /ansible:deploy-setup   Setup Ansible CI/CD pipeline"
    echo ""
}
...
main() {
    ...
    echo ""
    echo "=========================================="
    echo "Installing Ansible rules for Claude Code"
    echo "=========================================="
    echo ""
    ...
    if [ -z "$mode" ]; then
        ...
        case $installation_status in
            existing)
                ...
                ;;
            partial|none)
                log_info "No Ansible installation detected -> install mode"
                mode="install"
                ;;
        esac
    fi
    ...
    case $mode in
        install|update)
            log_info "Installing Ansible agents and commands..."
