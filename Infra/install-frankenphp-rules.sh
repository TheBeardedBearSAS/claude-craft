...
#
# Installs FrankenPHP agents and commands for infrastructure automation
...
TECH_NAME="FrankenPHP"
TECH_NAMESPACE="frankenphp"
lang="en"
...
Install/Update FrankenPHP agents and commands for Claude Code.
...
Examples:
    ./install-frankenphp-rules.sh --install ./my-project
    ./install-frankenphp-rules.sh --update
    ./install-frankenphp-rules.sh --lang=fr --install ./project
    ./install-frankenphp-rules.sh --dry-run

Description:
    This script installs FrankenPHP agents and commands for Claude Code.

    5 agents:
    - @frankenphp-architect    - FrankenPHP Caddyfile design, worker topology, and framework integration specialist
    - @frankenphp-deployment   - FrankenPHP deployment, Docker, Kubernetes, and CI/CD pipeline specialist
    - @frankenphp-debug        - FrankenPHP worker crashes, memory leaks, and Caddyfile error diagnostics specialist
    - @frankenphp-security     - FrankenPHP auto-TLS, ECH, PQC, and Caddyfile hardening specialist
    - @frankenphp-performance  - FrankenPHP worker tuning, thread autoscaling, Early Hints, and Mercure performance specialist
...
show_version() {
    echo "install-frankenphp-rules.sh version ${VERSION}"
    echo "FrankenPHP agents and commands for Claude Code"
}
...
create_backup() {
    local target_dir="$1"
    local dry_run="$2"
    local backup_dir="${target_dir}/.claude-backup-frankenphp-$(date +%Y%m%d-%H%M%S)"

    if [ -d "${target_dir}/.claude/commands/${TECH_NAMESPACE}" ] || [ -d "${target_dir}/.claude/agents" ]; then
        if [ "$dry_run" = "true" ]; then
            log_dry_run "Backup FrankenPHP files to ${backup_dir}"
        else
            mkdir -p "${backup_dir}/commands"
            mkdir -p "${backup_dir}/agents"
            cp -r "${target_dir}/.claude/commands/${TECH_NAMESPACE}" "${backup_dir}/commands/" 2>/dev/null || true
            # Only backup frankenphp-related agents
            for agent in frankenphp-architect frankenphp-deployment frankenphp-debug frankenphp-security frankenphp-performance; do
...
copy_agents() {
    ...
    if [ -d "$agt_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} FrankenPHP agents copied"
        fi
    fi
}
...
copy_commands() {
    ...
    if [ -d "$cmd_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} FrankenPHP commands copied"
        fi
    fi
}
...
show_summary() {
    ...
    echo "  |   |-- agents/             (${agents_count} FrankenPHP agents)"
    echo ""

    # List available commands
    if [ $commands_count -gt 0 ]; then
        echo "Available FrankenPHP commands:"
    ...
    if [ $agents_count -gt 0 ]; then
        echo "Available FrankenPHP agents:"
        for agent in frankenphp-architect frankenphp-deployment frankenphp-debug frankenphp-security frankenphp-performance; do
            if [ -f "${target_dir}/.claude/agents/${agent}.md" ]; then
                echo "  - @${agent}"
            fi
    ...
    echo "Usage examples:"
    echo "  @frankenphp-architect      Design FrankenPHP serving architecture"
    echo "  @frankenphp-security       Audit FrankenPHP security posture"
    echo "  /frankenphp:architecture   Design complete FrankenPHP architecture"
    echo "  /frankenphp:debug          Diagnose FrankenPHP worker issues"
    echo "  /frankenphp:deploy-setup   Setup FrankenPHP deployment pipeline"
    echo ""
}
...
main() {
    ...
    echo ""
    echo "=========================================="
    echo "Installing FrankenPHP rules for Claude Code"
    echo "=========================================="
    echo ""
    ...
    if [ -z "$mode" ]; then
        ...
        case $installation_status in
            existing)
                ...
                ;;
            partial|none)
                log_info "No FrankenPHP installation detected -> install mode"
                mode="install"
                ;;
        esac
    fi
    ...
    case $mode in
        install|update)
            log_info "Installing FrankenPHP agents and commands..."
