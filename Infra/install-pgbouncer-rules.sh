#!/bin/bash
# Multilingual installation script for Claude Code Infrastructure rules (PgBouncer)
# Version: 1.0.0
# Usage: ./install-pgbouncer-rules.sh [OPTIONS] [PROJECT_DIR]
#
# Installs PgBouncer agents and commands for infrastructure automation
...
Install/Update PgBouncer agents and commands for Claude Code.
...
Examples:
    ./install-pgbouncer-rules.sh --install ./my-project
    ./install-pgbouncer-rules.sh --update
    ./install-pgbouncer-rules.sh --lang=fr --install ./project
    ./install-pgbouncer-rules.sh --dry-run

Description:
    This script installs PgBouncer agents and commands for Claude Code.

    5 agents:
    - @pgbouncer-architect    - PgBouncer pool topology and sizing design specialist
    - @pgbouncer-deployment   - PgBouncer deployment and CI/CD pipeline specialist
    - @pgbouncer-debug        - PgBouncer connection issue diagnostics specialist
    - @pgbouncer-security     - PgBouncer authentication, TLS, and access control specialist
    - @pgbouncer-monitoring   - PgBouncer metrics, alerting, and performance tuning specialist
...
show_version() {
    echo "install-pgbouncer-rules.sh version ${VERSION}"
    echo "PgBouncer agents and commands for Claude Code"
}
...
create_backup() {
    local target_dir="$1"
    local dry_run="$2"
    local backup_dir="${target_dir}/.claude-backup-pgbouncer-$(date +%Y%m%d-%H%M%S)"

    if [ -d "${target_dir}/.claude/commands/${TECH_NAMESPACE}" ] || [ -d "${target_dir}/.claude/agents" ]; then
        if [ "$dry_run" = "true" ]; then
            log_dry_run "Backup PgBouncer files to ${backup_dir}"
        else
            mkdir -p "${backup_dir}/commands"
            mkdir -p "${backup_dir}/agents"
            cp -r "${target_dir}/.claude/commands/${TECH_NAMESPACE}" "${backup_dir}/commands/" 2>/dev/null || true
            # Only backup pgbouncer-related agents
            for agent in pgbouncer-architect pgbouncer-deployment pgbouncer-debug pgbouncer-security pgbouncer-monitoring; do
...
copy_agents() {
    ...
    if [ -d "$agt_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} PgBouncer agents copied"
        fi
    fi
}
...
copy_commands() {
    ...
    if [ -d "$cmd_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} PgBouncer commands copied"
        fi
    fi
}
...
show_summary() {
    ...
    echo "  |   |-- agents/             (${agents_count} PgBouncer agents)"
    echo ""

    # List available commands
    if [ $commands_count -gt 0 ]; then
        echo "Available PgBouncer commands:"
    ...
    if [ $agents_count -gt 0 ]; then
        echo "Available PgBouncer agents:"
        for agent in pgbouncer-architect pgbouncer-deployment pgbouncer-debug pgbouncer-security pgbouncer-monitoring; do
            if [ -f "${target_dir}/.claude/agents/${agent}.md" ]; then
                echo "  - @${agent}"
            fi
    ...
    echo "Usage examples:"
    echo "  @pgbouncer-architect      Design PgBouncer pool topology"
    echo "  @pgbouncer-security       Audit PgBouncer security posture"
    echo "  /pgbouncer:architecture   Design complete PgBouncer architecture"
    echo "  /pgbouncer:debug          Diagnose PgBouncer connection issues"
    echo "  /pgbouncer:deploy-setup   Setup PgBouncer deployment pipeline"
    echo ""
}
...
main() {
    ...
    echo ""
    echo "=========================================="
    echo "Installing PgBouncer rules for Claude Code"
    echo "=========================================="
    echo ""
    ...
    if [ -z "$mode" ]; then
        ...
        case $installation_status in
            existing)
                ...
                ;;
            partial|none)
                log_info "No PgBouncer installation detected -> install mode"
                mode="install"
                ;;
        esac
    fi
    ...
    case $mode in
        install|update)
            log_info "Installing PgBouncer agents and commands..."
