...
set -euo pipefail
IFS=$'\n\t'
# Multilingual installation script for Claude Code Infrastructure rules (Coolify)
# Version: 1.0.0
# Usage: ./install-coolify-rules.sh [OPTIONS] [PROJECT_DIR]
#
# Installs Coolify agents and commands for self-hosted PaaS deployment
...
Install/Update Coolify agents and commands for Claude Code.
...
Examples:
    ./install-coolify-rules.sh --install ./my-project
    ./install-coolify-rules.sh --update
    ./install-coolify-rules.sh --lang=fr --install ./project
    ./install-coolify-rules.sh --dry-run

Description:
    This script installs Coolify agents and commands for Claude Code.

    4 agents:
    - @coolify-architect     - Coolify infrastructure architect
    - @coolify-deployment    - Coolify deployment specialist
    - @coolify-debug         - Coolify troubleshooting specialist
    - @coolify-monitoring    - Coolify monitoring and backup specialist
    ...
    - /coolify:backup        - Configure and manage Coolify backups
    - /coolify:optimize      - Optimize Coolify deployment
EOF
}

show_version() {
    echo "install-coolify-rules.sh version ${VERSION}"
    echo "Coolify agents and commands for Claude Code"
}
...
create_backup() {
    local target_dir="$1"
    local dry_run="$2"
    local backup_dir="${target_dir}/.claude-backup-coolify-$(date +%Y%m%d-%H%M%S)"

    if [ -d "${target_dir}/.claude/commands/${TECH_NAMESPACE}" ] || [ -d "${target_dir}/.claude/agents" ]; then
        if [ "$dry_run" = "true" ]; then
            log_dry_run "Backup Coolify files to ${backup_dir}"
        else
            mkdir -p "${backup_dir}/commands"
            mkdir -p "${backup_dir}/agents"
            cp -r "${target_dir}/.claude/commands/${TECH_NAMESPACE}" "${backup_dir}/commands/" 2>/dev/null || true
            # Only backup coolify-related agents
            for agent in coolify-architect coolify-deployment coolify-debug coolify-monitoring; do
...
copy_agents() {
    ...
    if [ -d "$agt_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} Coolify agents copied"
        fi
    fi
}
...
copy_commands() {
    ...
    if [ -d "$cmd_dir" ]; then
        if [ "$dry_run" = "true" ]; then
            ...
            log_success "${count} Coolify commands copied"
        fi
    fi
}
...
show_summary() {
    ...
    echo "  |   |-- agents/             (${agents_count} Coolify agents)"
    echo ""

    # List available commands
    if [ $commands_count -gt 0 ]; then
        echo "Available Coolify commands:"
    ...
    echo "Usage examples:"
    echo "  @coolify-architect     Design Coolify infrastructure"
    echo "  @coolify-deployment    Configure and deploy applications"
    echo "  /coolify:setup         Initialize project for Coolify"
    echo "  /coolify:deploy        Deploy to Coolify instance"
    echo "  /coolify:debug         Diagnose deployment issues"
    echo ""
}
...
main() {
    ...
    echo ""
    echo "=========================================="
    echo "Installing Coolify rules for Claude Code"
    echo "=========================================="
    echo ""
    ...
    if [ -z "$mode" ]; then
        ...
        case $installation_status in
            existing)
                ...
                ;;
            partial|none)
                log_info "No Coolify installation detected -> install mode"
                mode="install"
                ;;
        esac
    fi
    ...
    case $mode in
        install|update)
            log_info "Installing Coolify agents and commands..."
