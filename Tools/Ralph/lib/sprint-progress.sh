...
update_sprint_progress() {
    ...
    local timestamp=$(date -Iseconds)
    sed -i "s/Last Update: .*/Last Update: $timestamp/" "$SPRINT_PROGRESS_FILE"
...
update_state_section() {
    ...
    [[ -n "$sprint" ]] && sed -i "s/\*\*Sprint\*\*: .*/\*\*Sprint\*\*: $sprint/" "$SPRINT_PROGRESS_FILE"
    [[ -n "$current_us" ]] && sed -i "s/\*\*Current US\*\*: .*/\*\*Current US\*\*: $current_us/" "$SPRINT_PROGRESS_FILE"
    [[ -n "$current_task" ]] && sed -i "s/\*\*Current Task\*\*: .*/\*\*Current Task\*\*: $current_task/" "$SPRINT_PROGRESS_FILE"
    [[ -n "$phase" ]] && sed -i "s/\*\*Phase\*\*: .*/\*\*Phase\*\*: $phase/" "$SPRINT_PROGRESS_FILE"
}
...
add_completed_task() {
    ...
    sed -i "/^## Completed Tasks$/a\\- [x] $task_info" "$SPRINT_PROGRESS_FILE"
}
...
add_key_decision() {
    ...
    sed -i "/^## Key Decisions$/a\\- $decision" "$SPRINT_PROGRESS_FILE"
}
...
add_modified_file() {
    ...
    sed -i "/^## Files Modified This Session$/a\\- $file_path" "$SPRINT_PROGRESS_FILE"
}
...
update_test_status() {
    ...
    local unit_status=$(echo "$test_info" | grep -oP 'unit:\K[^,]*' 2>/dev/null || echo "unknown")
    local integration_status=$(echo "$test_info" | grep -oP 'integration:\K[^,]*' 2>/dev/null || echo "unknown")

    sed -i "s/- Unit: .*/- Unit: $unit_status/" "$SPRINT_PROGRESS_FILE"
    sed -i "s/- Integration: .*/- Integration: $integration_status/" "$SPRINT_PROGRESS_FILE"
}
...
update_blockers() {
    ...
    # This is a simple replacement - for complex updates, use a temp file approach
    sed -i "s/^- None$/- $blockers/" "$SPRINT_PROGRESS_FILE" 2>/dev/null || true
}
...
increment_compact_count() {
    local current=$(grep -oP 'Compacts: \K\d+' "$SPRINT_PROGRESS_FILE" 2>/dev/null || echo "0")
    local new_count=$((current + 1))
    sed -i "s/Compacts: $current/Compacts: $new_count/" "$SPRINT_PROGRESS_FILE"
    print_verbose "Compact count incremented to $new_count"
}

increment_context_reset_count() {
    local current=$(grep -oP 'Context Resets: \K\d+' "$SPRINT_PROGRESS_FILE" 2>/dev/null || echo "0")
    local new_count=$((current + 1))
    sed -i "s/Context Resets: $current/Context Resets: $new_count/" "$SPRINT_PROGRESS_FILE"
...
save_sprint_progress() {
    ...
    # Create backup
    cp "$SPRINT_PROGRESS_FILE" "${SPRINT_PROGRESS_FILE}.backup"
